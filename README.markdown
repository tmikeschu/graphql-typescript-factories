[![NPM](https://img.shields.io/npm/v/@tmikeschu/graphql-typescript-factories)](https://www.npmjs.com/package/@tmikeschu/graphql-typescript-factories)
...
Somewhat tangentially, we've added first-class handling of [homebound](https://www.github.com/homebound-team)-specific "Enum Detail" pattern, where instead of returning enum values directly, we wrap the enum with a detail object that adds the string name, so that the client doesn't have to have its own boilerplate "enum to name" mapping. I.e. this schema:
